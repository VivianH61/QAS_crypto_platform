...
contract('QASToken', function(accounts){
	...
	it('initializes the contract with the correct values', function(){
		return QASToken.deployed().then(function(instance){
			...
			assert.equal(name,'QAS Token', 'has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol) {
			assert.equal(symbol, 'QAST', 'has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard) {
			assert.equal(standard, 'QAS Token v1.0', 'has the correct standard');
		});
	})
	...
	it('transfers token ownership', function() {
		return QASToken.deployed().then(function(instance) {
			...
			return tokenInstance.transfer.call(accounts[1], 99999999999999999999999); // the .call will not trigger a transaction. Without .call will transfer a transaction
	...
	it('handles delegated token transfers', function() {
		return QASToken.deployed().then(function(instance) {
			...
			assert.equal(receipt.logs[0].args._value, 10, 'logs the approval amount');
